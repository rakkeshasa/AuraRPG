...
{
	// 블루프린트에서 overlay widget controller을 부르기 위해
	if (APlayerController* PC = UGameplayStatics::GetPlayerController(WorldContextObject, 0))
	{
		// player controller에서는 hud접근이 가능했다
		OutAuraHUD = Cast<AAuraHUD>(PC->GetHUD());
		if (OutAuraHUD)
		{
			// 위젯 컨트롤러 가져오기
			// 위젯 컨트롤러는 4가지 속성이 필요하다
			// 컨트롤러, 스테이트, ASC, AttributeSet
...
{
	// AttributeMenuWidgetController 생성해주기
	FWidgetControllerParams WCParams;
	AAuraHUD* AuraHUD = nullptr;
...
void UAuraAbilitySystemLibrary::InitializeDefaultAttributes(const UObject* WorldContextObject, ECharacterClass CharacterClass, float Level, UAbilitySystemComponent* ASC)
{
	...
	// GameplayEffect 적용시키기
	// GameplayEffect가 MMC를 사용하는 즉시 Source에서 CombatInterface를 가져옴
	FGameplayEffectContextHandle PrimaryAttributesContextHandle = ASC->MakeEffectContext();
	PrimaryAttributesContextHandle.AddSourceObject(AvatarActor);
	const FGameplayEffectSpecHandle PrimaryAttributesSpecHandle = ASC->MakeOutgoingSpec(ClassDefaultInfo.PrimaryAttributes, Level, PrimaryAttributesContextHandle);
	// *PrimaryAttributesSpecHandle.Data.Get() 하는 이유
	// 받는 인자가 Handle이 아니라 EffectSpec이다.
...
void UAuraAbilitySystemLibrary::InitializeDefaultAttributesFromSaveData(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ULoadScreenSaveGame* SaveGame)
{
	...
	// ApplyDamageEffect()에서 setbycaller를 이용해본적이 있다.
	...
	// 저장된 정보로부터 스탯을 가져와 GE에 SetByCaller형식으로 적용해주기
...
void UAuraAbilitySystemLibrary::GiveStartupAbilities(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ECharacterClass CharacterClass)
{
	...
	// GiveStartupAbilities()는 서버에서만 유효하므로 
	// CharacterClassInfo를 가져오는 데 널 포인터를 가질 수 있다.
	if (CharacterClassInfo == nullptr) return;
...
{
	// Chapter 155
	// 커스텀 버전 EffectContext를 가져오기 위해 static_cast
	// static_cast는 *가 필요하다
...
{
	// 매개변수인 EffectContextHandle에 const가 없다
	// 이러한 매개변수는 블루프린트 노드에서 출력 파라미터로 나타난다.
	// UPARAM(ref)는 non-const여도 블루프린트에 입력 파라미터로 나타날 수 있게 해줌.
	...
	float Radius, const FVector& SphereOrigin)
{
	...
	SphereParams.AddIgnoredActors(ActorsToIgnore); // 무시할 Actor 배열
	...
	if (const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			// 액터가 내 전투 인터페이스를 구현하는지 확인
			// 액터가 살아 있는지 확인
			if (Overlap.GetActor()->Implements<UCombatInterface>() && !ICombatInterface::Execute_IsDead(Overlap.GetActor()))
			{
				// OutOverlappingActors에는 AI나 플레이어에게 조종받고 살아있는 아바타로 채워짐
				// Actor가 중복되지 않도록 AddUnique사용
